...
class StreamTest extends RSocketTestCase
{
    public function testSendRequestStream(): void
    {
        ...
        $connection->requestStream(3, 'dane');
    ...
    public function testReciveRequestResponse(): void
    {
        ...
        $reciveRequestResponse = new Promise(function (callable $resolve) use ($connection) {
            $connection->onRecivedRequest()->take(1)->subscribe(function (RequestStreamFrame $frame) use ($resolve, $connection) {
    ...
    public function testDoNotSendResponseWithoutResponseN(): void
    {
        ...
        $reciveRequestResponse = new Promise(function (callable $resolve) use ($connection, $testConnector) {
            $connection->onRecivedRequest()->take(1)->subscribe(function (RequestStreamFrame $frame) use ($resolve, $connection, $testConnector) {
                $this->assertEquals('data', $frame->getData());
